...
const MobileFooterDashboardMenuEditor = () => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        setConfig({
          dashboardFooterMenuIsPublished: settings.dashboardFooterMenuIsPublished !== false,
  ...
  const handleSubmitItem = () => {
    ...
    setConfig(prev => {
      ...
      return { ...prev, dashboardFooterMenuItems: nextItems };
    });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data?.data) {
        setConfig({
          dashboardFooterMenuIsPublished: data.data.dashboardFooterMenuIsPublished !== false,
      ...
      toast.success('Dashboard Navigation updated successfully');
    } catch {
    ...
    <section className="overflow-hidden rounded-sm border border-white/40 bg-white/10 p-4 text-white backdrop-blur-xl">
      ...
      <div className="mb-6 flex items-center justify-between rounded-sm border border-white/10 bg-white/5 p-4">
        <div>
          ...
          <p className="text-xs text-white/50">Show or hide the dashboard navigation on mobile devices.</p>
        </div>
        <Switch
          checked={config.dashboardFooterMenuIsPublished}
          onCheckedChange={value => setConfig(prev => ({ ...prev, dashboardFooterMenuIsPublished: value }))}
        />
      </div>
