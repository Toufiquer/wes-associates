...
const MOBILE_BOTTOM_MENU_API = '/api/footer-settings/mobile-bottom-menu/v1';

type MobileFooterConfig = Pick<BrandSettings, 'mobileBottomMenuIsPublished' | 'mobileMenuVariant' | 'mobileMenuItems'>;
...
const defaultConfig: MobileFooterConfig = {
  mobileBottomMenuIsPublished: true,
...
const MobileNaviation = () => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        setConfig({
          mobileBottomMenuIsPublished: brandSettings.mobileBottomMenuIsPublished !== false,
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const response = await fetch(MOBILE_BOTTOM_MENU_API, {
        method: 'POST',
      ...
      if (data?.data) {
        setConfig(prev => ({
          ...prev,
          mobileBottomMenuIsPublished: data.data.mobileBottomMenuIsPublished !== false,
          mobileMenuVariant: data.data.mobileMenuVariant,
          mobileMenuItems: data.data.mobileMenuItems,
        }));
      }
      window.dispatchEvent(new Event('mobile-bottom-menu-updated'));
      toast.success('Mobile Navigation updated successfully');
    } catch {
    ...
    <section className="overflow-hidden rounded-sm border border-white/40 bg-white/10 p-4 text-white backdrop-blur-xl">
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between rounded-sm border border-white/10 bg-white/5 p-4">
          <div>
            <h3 className="text-sm font-bold">Visible in Mobile</h3>
            <p className="text-xs text-white/50">Show or hide the mobile navigation on the website.</p>
          </div>
          <Switch
            checked={config.mobileBottomMenuIsPublished}
            onCheckedChange={value => setConfig(prev => ({ ...prev, mobileBottomMenuIsPublished: value }))}
          />
        </div>
