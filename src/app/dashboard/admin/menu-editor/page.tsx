...
import { IMenuItem } from '@/app/api/menu/primary-menu/model';
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-purple-950 to-slate-950 p-2 font-sans text-slate-100">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto space-y-6">
        <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-xl p-4 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 relative z-10">
            ...
            <div className="flex flex-wrap gap-2 w-full lg:w-auto">
              {[
                ...
                <Button
                  key={tab.key}
                  onClick={() => setActiveTopEditor(tab.key)}
                  variant="outlineGlassy"
                  size="sm"
                  aria-pressed={activeTopEditor === tab.key}
                  className={activeTopEditor === tab.key ? 'bg-white/20 border-white/30 text-white shadow-lg shadow-blue-950/20' : ''}
                >
                  {tab.label}
                </Button>
              ))}
              {activeTopEditor && (
                <Button onClick={() => setActiveTopEditor(null)} variant="outlineFire" size="sm">
                  Close X
                </Button>
              )}
        ...
        <AnimatePresence>
          {activeTopEditor === 'search' && (
            <motion.div
              ...
              className="overflow-hidden"
            >
              <SearchMenuButtonEditor />
            </motion.div>
          )}
        </AnimatePresence>

        <AnimatePresence>
          {activeTopEditor === 'mobile-menu' && (
            <motion.div
              key="mobile-menu-editor"
              initial={{ opacity: 0, height: 0 }}
              animate={{ opacity: 1, height: 'auto' }}
              exit={{ opacity: 0, height: 0 }}
              className="overflow-hidden"
