...
const MutationSection8 = ({ data, onSubmit }: Section8FormProps) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-zinc-950 to-cyan-950/40 text-zinc-100 p-4 md:p-8 font-sans selection:bg-cyan-500/30">
      <div className="max-w-7xl mx-auto bg-zinc-900/50 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl backdrop-blur-sm">
        <div className="p-6 border-b border-zinc-800 bg-zinc-900/80 backdrop-blur flex items-center gap-3 relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-r from-cyan-600/5 to-emerald-600/5 pointer-events-none" />
          <div className="p-2 bg-cyan-500/10 rounded-lg border border-cyan-500/20 z-10">
            <Globe className="text-cyan-400" size={24} />
          </div>
          <div className="z-10">
            <h2 className="text-xl font-bold bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent">Edit Study Destinations</h2>
        ...
        <div className="p-6 md:p-8 grid grid-cols-1 xl:grid-cols-3 gap-8">
          <div className="xl:col-span-1 space-y-6">
              ...
              <div className="bg-zinc-950/50 p-5 rounded-xl border border-zinc-800/50 space-y-5 shadow-inner">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <Globe className="absolute left-3 top-2.5 h-4 w-4 text-zinc-500 group-hover:text-cyan-400 transition-colors" />
                    <Input
                      value={formData.country}
                      onChange={e => updateRootField('country', e.target.value)}
                      className="bg-zinc-900 border-zinc-800 pl-9 focus:border-cyan-500 transition-all duration-300 focus:ring-1 focus:ring-cyan-500/20"
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      className="bg-zinc-900 border-zinc-800 focus:border-cyan-500 transition-all"
                      placeholder="Add city..."
                    />
                    <Button onClick={addCity} className="bg-cyan-600 hover:bg-cyan-700 shadow-lg shadow-cyan-900/20">
                      <Plus size={16} />
                    </Button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {formData.city.map((item, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center gap-1.5 px-3 py-1 bg-zinc-800/50 text-zinc-200 rounded-md text-xs font-medium border border-zinc-700 hover:border-cyan-500/30 hover:bg-zinc-800 transition-colors"
                      >
                        {item}
            ...
            <div className="sticky top-6 bg-gradient-to-br from-cyan-900/10 via-zinc-900/50 to-emerald-900/10 border border-cyan-500/10 rounded-xl p-6 text-center space-y-4 backdrop-blur-md">
              <div className="w-12 h-12 bg-cyan-500/10 rounded-full flex items-center justify-center mx-auto text-cyan-400 ring-4 ring-cyan-500/5">
                <Building2 size={24} />
              </div>
              <div>
                <h3 className="text-base font-semibold text-zinc-200">Universities</h3>
                <p className="text-zinc-500 text-xs mt-1">
                  You have <span className="text-cyan-400 font-bold">{formData.universitys.length}</span> universities listed.
                </p>
              </div>
              <Button
                onClick={addUniversity}
                variant="outline"
                className="w-full bg-zinc-900/80 border-cyan-500/30 text-cyan-300 hover:bg-cyan-500 hover:text-white transition-all hover:shadow-lg hover:shadow-cyan-500/20"
              >
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            {formData.universitys.length === 0 && (
              <div className="text-center py-16 bg-zinc-950/30 border border-dashed border-zinc-800 rounded-2xl flex flex-col items-center justify-center gap-3">
                ...
                <Button onClick={addUniversity} variant="link" className="text-cyan-400 hover:text-cyan-300">
              ...
              {formData.universitys.map((uni, index) => (
                <div
                  ...
                  className={`
                    ...
                    ${expandedUni === uni.id ? 'border-cyan-500/30 ring-1 ring-cyan-500/10 bg-zinc-900/40 shadow-xl' : 'border-zinc-800/50 hover:border-zinc-700 hover:bg-zinc-900/20'}
                  `}
                >
                  <div className="p-4 flex items-center justify-between cursor-pointer select-none" onClick={() => toggleExpand(uni.id)}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className={`text-base font-medium transition-colors ${expandedUni === uni.id ? 'text-cyan-200' : 'text-zinc-200'}`}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div
                        className={`p-1.5 rounded-full bg-zinc-900 border border-zinc-800 text-zinc-500 transition-transform duration-300 ${expandedUni === uni.id ? 'rotate-180 text-cyan-400 border-cyan-500/30' : ''}`}
                      >
                        <ChevronDown size={14} />
                      </div>
                    </div>
                        ...
                        <div className="space-y-5">
                          <div className="space-y-2">
                            ...
                            <div className="relative group/input">
                              <Building2 className="absolute left-3 top-2.5 h-4 w-4 text-zinc-500 group-hover/input:text-cyan-400 transition-colors" />
                              <Input
                                value={uni.name}
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="relative group/select">
                              <MapPin className="absolute left-3 top-2.5 h-4 w-4 text-zinc-500 z-10 group-hover/select:text-cyan-400 transition-colors" />
                              <select
                                value={uni.location}
                                onChange={e => handleUniversityChange(index, 'location', e.target.value)}
                                className="w-full bg-zinc-900 border border-zinc-800 text-zinc-200 text-sm rounded-md h-10 pl-9 pr-10 appearance-none focus:outline-none focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 transition-all cursor-pointer hover:bg-zinc-900/80"
                              >
                          ...
                          <div className="space-y-2">
                            ...
                            <Textarea
                              ...
                              className="bg-zinc-900 border-zinc-800 min-h-[100px] resize-none focus:ring-1 focus:ring-cyan-500"
                      ...
                      <div className="mt-8 space-y-4">
                        <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                          ...
                          <Button
                            ...
                            className="bg-zinc-900 border-zinc-800 text-zinc-300 hover:bg-cyan-500 hover:text-white h-8 text-xs transition-all"
                          >
                                ...
                                <div className="space-y-2">
                                  <div className="flex items-center justify-between">
                                    <Label className="text-[10px] font-bold text-zinc-500 flex items-center gap-2 uppercase tracking-wide">
                                      <Award size={12} className="text-cyan-400" /> Degree Levels Info
                                    </Label>
                                    <Button
                                      size="sm"
                                      variant="ghost"
        ...
        <div className="p-6 border-t border-zinc-800 bg-zinc-900/80 backdrop-blur flex justify-between items-center">
          ...
          <Button onClick={handleSave} className="bg-cyan-600 hover:bg-cyan-500 text-white shadow-lg shadow-cyan-900/20 border border-cyan-500/20">
