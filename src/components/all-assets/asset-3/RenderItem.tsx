...
const RenderItem = ({ item, settings }: RenderItemProps) => {
  ...
  const detailUrl = item.url || (item.sourceProductId ? `/template?id=${item.sourceProductId}` : `/template?id=${item.id}`);
  const categoryLabel = item.category?.length ? item.category.join(', ') : '';
  ...
  return (
    <div className="group relative flex h-full w-full min-w-0 flex-col overflow-hidden rounded-sm border border-gray-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:border-blue-200 hover:shadow-xl hover:shadow-blue-100/60">
      {/* Shimmer accent line */}
      ...
      <div className="flex min-h-0 flex-1 flex-col p-2.5">
        ...
        {categoryLabel && <p className="mt-1 line-clamp-1 text-[10px] font-semibold uppercase text-blue-500/70">{categoryLabel}</p>}
        ...
        <div className="mt-auto flex translate-y-1 gap-1 pt-3 opacity-80 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
          <button
            ...
            className="flex flex-1 items-center justify-center gap-1.5 rounded bg-blue-600 py-1.5 text-[11px] font-bold text-white transition-all duration-200 hover:bg-blue-700 active:scale-[0.98] md:text-sm"
          >
