...
import { AssetGridLayout, AssetMobileGridLayout, AssetSortMode, defaultDataAsset3, IAssetData, ProductItem, ProductsResponse, TemplateItem, templateImagePlaceholder } from './data';
...
const normalizeSettings = (data?: IAssetData): IAssetData => ({
  ...defaultDataAsset3,
  ...data,
  title: data?.title || data?.sectionTitle || defaultDataAsset3.title,
  sortMode: data?.sortMode || defaultDataAsset3.sortMode,
  gridLayout: data?.gridLayout || defaultDataAsset3.gridLayout,
  mobileGridLayout: data?.mobileGridLayout || defaultDataAsset3.mobileGridLayout,
  showSeeMore: data?.showSeeMore ?? defaultDataAsset3.showSeeMore,
  showBottomNavigation: data?.showBottomNavigation ?? defaultDataAsset3.showBottomNavigation,
  seeMore: {
    ...defaultDataAsset3.seeMore,
    ...(data?.seeMore || {}),
    name: data?.seeMore?.name || data?.viewMoreText || defaultDataAsset3.seeMore.name,
  },
  templates: data?.templates || defaultDataAsset3.templates,
});
...
const MutationAsset3 = ({ data, onSubmit }: AssetFormProps) => {
  ...
  const importedProductsGridClassName = mutationGridLayoutClasses[settings.gridLayout] || mutationGridLayoutClasses[defaultDataAsset3.gridLayout];
      ...
      <div className={panelClass}>
        ...
        <div className="mt-5 flex flex-col gap-4 border-t border-white/10 pt-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-stretch gap-2.5">
            <label className="flex min-h-11 items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/20 px-3.5 text-sm font-bold text-white">
              <span className="whitespace-nowrap">Bottom Nav</span>
              <Switch checked={settings.showBottomNavigation} onCheckedChange={checked => updateSettings({ showBottomNavigation: checked })} />
            </label>
            ...
            {sortedTemplates.map(item => {
              ...
              return (
                <div
                  ...
                  className={`group relative min-w-0 overflow-hidden rounded-xl border bg-white/[0.04] backdrop-blur-md transition-all duration-200 ${
                ...
                >
                  ...
                  {isCompact ? (
                    ...
                    <div className="flex flex-1 flex-col gap-1.5 p-3">
                      <h4 className="min-h-10 line-clamp-2 text-sm font-semibold leading-snug text-white">{item.title || 'Untitled Product'}</h4>
                      ...
                      <button
                        type="button"
                        onClick={() => removeTemplate(item.id)}
                        className="mt-auto inline-flex w-full items-center justify-center gap-1.5 rounded-lg border border-red-300/20 bg-red-500/10 px-3 py-2 text-xs font-bold text-red-200 transition-colors hover:bg-red-500/20 hover:text-white"
                        aria-label={`Remove ${item.title}`}
                      >
                        <Trash2 size={13} /> Remove
                      </button>
