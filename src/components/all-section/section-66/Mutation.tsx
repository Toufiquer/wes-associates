...
import { defaultDataSection66, type ISection66Data } from './data';
...
const colorFields: { field: Exclude<keyof ISection66Data, 'eyebrow' | 'title' | 'description' | 'faqs'>; label: string }[] = [
...
const MutationSection66 = ({ data, onSubmit }: Section66FormProps) => {
  const [formData, setFormData] = useState<ISection66Data>({ ...defaultDataSection66, faqs: defaultDataSection66.faqs.map(faq => ({ ...faq })) });

  useEffect(() => {
    setFormData({ ...defaultDataSection66, ...(data || {}), faqs: (data?.faqs || defaultDataSection66.faqs).map(faq => ({ ...faq })) });
  }, [data]);
