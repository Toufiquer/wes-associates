...
import { ImageIcon, Palette, Plane, Save, Type } from 'lucide-react';
...
import { defaultDataSection68, type ISection68Data } from './data';

export interface Section68FormProps {
  data?: ISection68Data;
  onSubmit: (values: ISection68Data) => void;
}
...
const MutationSection68 = ({ data, onSubmit }: Section68FormProps) => {
  const [formData, setFormData] = useState<ISection68Data>({ ...defaultDataSection68, paragraphs: [...defaultDataSection68.paragraphs] });

  useEffect(() => {
    setFormData({ ...defaultDataSection68, ...(data || {}), paragraphs: [...(data?.paragraphs || defaultDataSection68.paragraphs)] });
  }, [data]);

  const updateField = <K extends keyof ISection68Data>(field: K, value: ISection68Data[K]) => {
              ...
              <div className="space-y-4 rounded-2xl border border-zinc-800 bg-zinc-950/45 p-5">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={formData.titleSuffix} onChange={event => updateField('titleSuffix', event.target.value)} className={fieldClassName} />
                  </div>
                </div>
...
export default MutationSection68;
