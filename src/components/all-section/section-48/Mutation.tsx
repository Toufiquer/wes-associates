...
import { defaultDataSection48 } from './data';

export interface Section48FormProps {
  data?: ISection48Data;
  onSubmit: (values: ISection48Data) => void;
}

const MutationSection48 = ({ data, onSubmit }: Section48FormProps) => {
  const [formData, setFormData] = useState<ISection48Data>({ ...defaultDataSection48 });
  const [cityInput, setCityInput] = useState('');
  const [expandedUni, setExpandedUni] = useState<string | null>(defaultDataSection48.universitys[0]?.id || null);
  ...
  const updateRootField = (field: keyof ISection48Data, value: any) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const handleUniversityChange = (index: number, field: keyof ISection48Data['universitys'][0], value: any) => {
