/*
|-----------------------------------------
| Admission process editor for Section 56
|-----------------------------------------
*/
...
import { ListChecks, Palette, Plus, Save, Trash2, Type } from 'lucide-react';
...
import { defaultDataSection56, type ISection56Data } from './data';

export interface Section56FormProps {
  data?: ISection56Data;
  onSubmit: (values: ISection56Data) => void;
}

const colorFields: { field: Exclude<keyof ISection56Data, 'eyebrow' | 'title' | 'description' | 'steps'>; label: string }[] = [
  { field: 'backgroundColor', label: 'Background' },
  { field: 'cardColor', label: 'Cards' },
  { field: 'accentColor', label: 'Accent' },
  { field: 'markerColor', label: 'Number markers' },
  { field: 'headingColor', label: 'Headings' },
  { field: 'textColor', label: 'Body text' },
  { field: 'borderColor', label: 'Borders' },
];

const MutationSection56 = ({ data, onSubmit }: Section56FormProps) => {
  const [formData, setFormData] = useState<ISection56Data>({ ...defaultDataSection56, steps: defaultDataSection56.steps.map(step => ({ ...step })) });

  useEffect(() => {
    setFormData({ ...defaultDataSection56, ...(data || {}), steps: (data?.steps || defaultDataSection56.steps).map(step => ({ ...step })) });
  }, [data]);

  const updateStep = (index: number, field: 'number' | 'title' | 'description', value: string) =>
    setFormData(current => ({ ...current, steps: current.steps.map((step, stepIndex) => (stepIndex === index ? { ...step, [field]: value } : step)) }));
  const addStep = () => setFormData(current => ({ ...current, steps: [...current.steps, { number: String(current.steps.length + 1).padStart(2, '0'), title: 'New process step', description: 'Describe this stage of the admission process.' }] }));
    ...
    <div className="min-h-screen bg-zinc-950 p-4 text-zinc-100 md:p-8">
      <div className="mx-auto max-w-6xl overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-900/60 shadow-2xl">
        <header className="flex items-center gap-3 border-b border-zinc-800 bg-zinc-900/90 p-6">
          <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-2.5"><ListChecks className="h-6 w-6 text-red-400" /></div>
          <div><h2 className="text-xl font-bold">Edit Admission Process</h2><p className="text-sm text-zinc-400">Manage the process introduction, ordered steps, and visual theme.</p></div>
        </header>
        ...
        <div className="space-y-8 p-6 md:p-8">
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between gap-4">
              <h3 className="text-sm font-bold uppercase tracking-[0.16em] text-zinc-400">Process steps</h3>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {formData.steps.map((step, index) => (
                <article key={index} className="rounded-2xl border border-zinc-800 bg-zinc-950/55 p-5">
                  <div className="mb-4 flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-widest text-zinc-500">Step {index + 1}</span><Button type="button" variant="ghost" size="icon" onClick={() => removeStep(index)} disabled={formData.steps.length === 1} className="h-8 w-8 text-zinc-500 hover:bg-red-500/10 hover:text-red-400"><Trash2 className="h-4 w-4" /><span className="sr-only">Remove step {index + 1}</span></Button></div>
                  <div className="grid gap-4 sm:grid-cols-[6rem_1fr]">
                    <div className="space-y-2"><Label>Number</Label><Input value={step.number} maxLength={3} onChange={event => updateStep(index, 'number', event.target.value)} className={fieldClassName} /></div>
                    <div className="space-y-2"><Label>Title</Label><Input value={step.title} onChange={event => updateStep(index, 'title', event.target.value)} className={fieldClassName} /></div>
                    <div className="space-y-2 sm:col-span-2"><Label>Description</Label><Textarea value={step.description} onChange={event => updateStep(index, 'description', event.target.value)} className={`${fieldClassName} min-h-20 resize-y`} /></div>
                  </div>
                </article>
              ))}
            </div>
...
export default MutationSection56;
