...
const defaultConfig: MobileBottomMenuConfig = {
...
const mobileMenuIconFallbacks: LucideIcon[] = [Home, FolderKanban, ShoppingCart, LogIn, Phone];
...
const isCartMenuItem = (item: MobileMenuConfigItem) => item.path === '/cart' || item.name.toLowerCase() === 'cart';
  ...
  const fallbackItems: MobileMenuConfigItem[] =
    config.mobileMenuVariant === '5-icon'
      ? [
          ...
          { id: 3, logo: 'ShoppingCart', name: 'Cart', path: '/cart' },
          { id: 4, logo: 'LogIn', name: 'Login', path: '/login' },
          { id: 5, logo: 'Phone', name: 'Contact', path: '/contact' },
        ]
      : [
          { id: 1, logo: 'Home', name: 'Home', path: '/' },
          { id: 2, logo: 'FolderKanban', name: 'Assets', path: '/dashboard/assets/products' },
...
const MobileNavigation = () => {
  ...
  useEffect(() => {
    ...
    void syncSettings();
    window.addEventListener('mobile-bottom-menu-updated', syncSettings);
    return () => window.removeEventListener('mobile-bottom-menu-updated', syncSettings);
  }, []);
  ...
  const displayItems = getMobileMenuItems(settings);
    ...
    <div className="fixed inset-x-0 bottom-0 z-[130] block px-3 pb-[max(env(safe-area-inset-bottom),0.5rem)] pt-2 lg:hidden">
      <div
        className={`mx-auto max-w-md rounded-2xl border border-slate-200 bg-white px-2 text-slate-700 shadow-[0_-10px_30px_rgba(15,23,42,0.16)] ${
          settings.mobileMenuVariant === '5-icon' ? 'py-[5px]' : 'py-2'
        }`}
      >
        <div className="grid items-end gap-2" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
          {items.map((item, index) => {
            ...
            const isMiddleIcon = settings.mobileMenuVariant === '5-icon' && index === 2;
            return (
              <Link key={`${item.id}-${item.path}`} href={item.path} className="group relative flex min-w-0 justify-center">
                ...
                >
                  ...
                  >
                    ...
                    {isCartMenuItem(item) && <CartQuantityBadge quantity={cartQuantity} className="h-4 min-w-4 px-1 text-[9px]" />}
                  </span>
